...
class GetCaptcha(object):
    ...
    def getProxyIp(self):
        try:
            url = 'http://api.ip.data5u.com/api/get.shtml?order=1f78f05122561122a36b4998a661bd0c&num=100&an1=1&an2=2&an3=3&sp1=1&sp2=2&sp3=3&sort=1&system=1&distinct=0&rettype=1&seprator=%0D%0A'
            result = self.s.get(url=url)
            res_list = result.text.strip().split('\r\n')
            proxy_ip = random.choice(res_list)
            proxyAddr = 'http://' + proxy_ip
            proxies = {'http': proxyAddr}
            return proxies
        except Exception, e:
            error_msg = "request proxy error: " + str(e)
            self.log_define(error_msg)
    ...
    def getHomePage(self):
        ...
        result = self.s.get(url,proxies=self.proxy)
    ...
    def getAccountPage(self):
        ...
        result = self.s.get(url,headers=header,proxies=self.proxy)
        self._cookies = result.cookies

        # for i in result.text.split('\n'):
        #     if r'scnt:' in i:
        #         scnt = i.strip().split()[1][1:-2]
    ...
    def captcha(self):
        ...
        pyload = {'type':'IMAGE'}
        result = self.s.post(url,json=pyload,proxies=self.proxy)
    ...
    def postAppleid(self):
        ...
        self.s.headers = self.header()
        result = self.s.post(url,json=payload,proxies=self.proxy)
        if result.status_code > 209:
    ...
    def putEmailCaptchaResult(self,passwd):
        ...
        self.s.headers = self.header()
        result = self.s.put(url, json=payload,proxies=self.proxy)
    ...
    def postRegisterForm(self):
        ...
        if result.status_code > 209:
            ...
            resultLoad = json.loads(result.text)['service_errors'][0]['code']
            res = self.s.get("http://members.3322.org/dyndns/getip",proxies=self.proxy)
            self.log_define("current ip proxy is: "+str(res.text))
            raise NameError(resultLoad)
        else:
    ...
    def imageResolution(self):
        ...
        header = {
            ...
            'Authorization': 'APPCODE 6d051550d66d44c442e6d7'
        }
...
def main(appleid,password):
    getCaptcha = GetCaptcha(appleid, '123TabcdE', 'CHN', 'ba', 'jie', '1993-12-21',
